...
# Casino socket is created
    ...
    s.listen(100)
# Connection with client socket is made
...
# Data about number of players and number of rounds is received from client
    print("Number of players ", unpacked_data[0])
    print("Number of rounds ", unpacked_data[1])
    n = unpacked_data[0]
    m = unpacked_data[1]

    packer_rounds = struct.Struct('I')
# A list of connections to player sockets is created
# Data of the number of rounds is sent to each player
    rounds = (m,)
    packed_rounds = packer_rounds.pack(*rounds)
    socket_list = []
    points = [0]*n
    for i in range(n):
        play, address = s.accept()
        socket_list.append((play, address))
        socket_list[i][0].sendall(packed_rounds)
    for j in range(1, m+1):
        ...
        print("Card values generated (in range 1 to 13) : ")
        print(list_num)
...
# And the queue elements are then stored in a list for further result calculation
        ...
        for k in range(n):
            ...
            thrds.append(Thread(target=send_and_receive, args=(
                socket_list[k][0], packed_data, queue)))
...
# And result is sent to client as well
        for k in range(n):
            if max(max_val) == max_val[k]:
                print("Round ", j, " winner is Player", k+1)
                client.send(bytes("Round " + str(j) +
                            " winner is player " + str(k+1), "utf-8"))
                points[k] = points[k] + 1
# From the points of each player winner of game is found
# and result is sent to each player as well as the client

    for k in range(n):
        thrds_result = []
        if max(points) == points[k]:
            client.send(bytes("Player " + str(k+1) +
                        " wins the game", "utf-8"))
            print("Player " + str(k+1) + " wins the game")
            for j in range(n):
                thrds_result.append(
                    Thread(target=send_result, args=(socket_list[j][0], k)))
            for j in range(n):
